...
int main() {
    ...
    int turtle_tick_row = 0;
    int turtle_tick_column = 0;
    // Set up mode
    for (i = 0; i < 9; i++) { 
    ...
    printf("Enter %d coordinate pairs, which will position the turtles in their corresponding places:\n", turtles);
    int turtle_arr_row[turtles];
    int turtle_arr_column[turtles];
    int turtle_bool_arr[turtles];
    for (d = 0; d < turtles; d++){
        scanf("%d %d", &turtle_row, &turtle_column);
        if (turtle_row > 0 && turtle_row < 8 && turtle_column >= 0 && turtle_column <= 8) {
            turtle_arr_row[turtle_tick_row] = turtle_row;
            turtle_arr_column[turtle_tick_column] = turtle_column;
            game_board[turtle_row][turtle_column].type = TURTLE;
            game_board[turtle_row][turtle_column].occupied = 0;
            turtle_tick_row += 1;
            turtle_tick_column += 1;
        }   
        else {
            continue;
        }
    ...
    for (int i = 0; i < turtles; i++){
        printf("%d ", turtle_arr_column[i]);
    }

    for (int i = 0; i < turtles; i++){
        printf("%d ", turtle_arr_row[i]);
    }
    ...
    while(1) {
        ...
        switch (command) { 
            case 'l':
                ...
                int turtle_row_identifier[SIZE];

                for (int i = 0; i < turtles; i++){
                    if (log_row == turtle_arr_row[i]) {
                        turtle_row_identifier[turtle_arr_row[i]] = 1;

                    } else {
                        turtle_row_identifier[turtle_arr_row[i]] = 0; 
                    }        
                ...
                for (int j = 0; j < turtles; j++)   
                    for (l = log_start; l <= log_end; l++) { 
                        if (turtle_row_identifier[log_row] != 1){
